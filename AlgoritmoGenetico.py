...
def genetic_algorithm(objective_function, dimension, lower_bound, upper_bound, population_size, mutation_rate, crossover_rate):
    ...
    for generation in range(num_generations):
        ...
        best_solutions.append((best_individual, best_fitness, generation))
    
    # Devolver el mejor resultado, la historia de la convergencia y las mejores soluciones por generación
    return best_individual, best_fitness, best_fitness_history, best_solutions
        ...
        for mut_rate in mutation_rates:
            for cross_rate in crossover_rates:
                ...
                for run in range(num_runs):
                    ...
                    best_solution, best_fitness, best_fitness_history, best_solutions = genetic_algorithm(
                    ...
                    print(f'Valor de la función objetivo: {best_fitness}')
                    print(f'Generación o iteración: {best_solutions[-1][2]}')
                    print(f'Tiempo CPU: {cpu_time} segundos')
                ...
                print(f'{problem["name"]} - PSize: {pop_size}, MRate: {mut_rate}, CRate: {cross_rate}, Mean Fitness: {mean_fitness}, Std Fitness: {std_fitness}, Best Fitness: {min(fitness_results)}, Mean CPU Time: {mean_cpu_time}')
...
for problem in problems:
    ...
    print(f'  Tiempo CPU promedio: {best_result["mean_cpu_time"]}')

    # Gráfico de convergencia para la mejor configuración
    cpu_times = []
    for run in range(num_runs):
        start_time = time.process_time()
        _, _, best_fitness_history, _ = genetic_algorithm(
            problem["objective_function"], 
            problem["dimension"], 
            problem["lower_bound"], 
            problem["upper_bound"],
        ...
        cpu_times.append(cpu_time)
        if run < 2:
            plt.plot(best_fitness_history, label=f'Ejecución {run + 1} - CPU: {cpu_time:.2f} s')
    ...
    plt.legend()
    plt.show()
    print(f'Tiempos de CPU de las ejecuciones mostradas: {cpu_times[0]:.2f} s, {cpu_times[1]:.2f} s')
